...
`defineModel` 是 Vue 3.4 版本中引入的一个编译器宏，用于简化父子组件之间的双向数据绑定。它使得组件数据可以通过 `v-model` 绑定到父组件，从而实现数据的双向同步。
...
<!-- 子组件y1 -->
<template>
  y1: <input type="text" v-model="values">
</template>

<script setup>

const values = defineModel('rules')  // rules在父组件使用时，v-model:rules
</script>
```
...
<template>
  {{ rule }}  <!-- 此处显示从子组件获取的值 -->
  <y1 v-model:rules="rule"/>
</template>

<script setup>
import y1 from '@/demoComponents/y1.vue' // 导入子组件y1
import {ref} from 'vue'  // 从vue中导入ref

const rule = ref()  // 声明一个响应式数据rule，将子组件中获取的数据绑定到rule
